...
def downloadsocks(choice):
    ...
    if choice == "4":
        ...
        try:
            ...
            pass
        try:
            r = requests.get('https://ghostbin.com/hSC41/raw', timeout=5)
            f.write(r.content)
            f.close()
        except:
            pass
    ...
    if choice == "5":
        ...
        try:
            ...
            pass
        try:
            r = requests.get('https://ghostbin.com/7ag69/raw', timeout=5)
            f.write(r.content)
            f.close()
        except:
            pass
    ...
    if choice == "1":
        ...
        try:
            ...
            pass
        try:
            r = requests.get('https://ghostbin.com/miWSi/raw', timeout=5)
            f.write(r.content)
            f.close()
        except:
            pass
        try:
            ...
            pass
        try:
            r = requests.get("https://ghostbin.com/kgZn2/raw", timeout=5)
            f.write(r.content)
            f.close()
        except:
            pass
...
def proxydl(out_file, socks_type):
    ...
    print(f"{lcy}[{green_color}-{lcy}] {white_color}Downloading {green_color}{socktyper} {white_color}proxy..")
    downloadsocks(choice)
    proxies = open(str(out_file)).readlines()
    print(f"{lcy}[{green_color}-{lcy}] {white_color}Checking {green_color}{socktyper} {white_color}proxy..")
    check_list(out_file)
    check_socks(ms)
...
{white_color}CFB, BYPASS, GET, POST, OVH, STRESS, OSTRESS, DYN, SLOW, HEAD, HIT, NULL, COOKIE, BRUST, PPS, EVEN, GSB, DGB, AVB

{lcy}[{green_color}Layer 4{lcy}]
{white_color}TCP, UDP, SYN, VSE, MEM, NTP

{lcy}[{green_color}Layer 3{lcy}]
{white_color}POD, ICMP

{lcy}[{green_color}Other{lcy}]
{white_color}CFIP, DNS, PING, CHECK, DSTAT, INFO, STOP, TOOLS, HELP

{lcy}[{green_color}Usage{lcy}]> {white_color}python3 gaskeun.py <method> <url> <socks_type5.4.1> <threads> <proxylist> <multiple> <timer>""")
